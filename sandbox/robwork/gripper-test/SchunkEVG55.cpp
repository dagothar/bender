...
const float SchunkEVG55::HOMEPOS = 0.034f;
const float SchunkEVG55::VEL = 0.07f;
const float SchunkEVG55::ACC = 0.035f;
const float SchunkEVG55::MAXPOS = 0.068f;
const float SchunkEVG55::MAXVEL = 0.095f;
const float SchunkEVG55::MAXACC = 0.039f;
const float SchunkEVG55::MAXCUR = 8.5f;



// Constructor
SchunkEVG55::SchunkEVG55() : 
...
bool SchunkEVG55::connectSerial(const std::string& port) {
	RW_WARN("connectSerial() {");
	if(initialize(port)) {
...
// Connect to gripper
bool SchunkEVG55::initialize(const std::string& port) {
	RW_WARN("initialize() {");
	...
	// Make a cube port
	RW_WARN("making cube port");
	std::vector<rwhw::Cube*> cubes;
	try {
		RW_WARN("make()");
		_cubePort = CubePort::make(_port);
		RW_WARN("getCubes()");
		cubes = rwhw::Cube::getCubes(0, 20, _cubePort);
	} catch(rw::common::Exception& e) {
...
bool SchunkEVG55::getQ(rw::math::Q &q) {
	_port->clean();
...
bool SchunkEVG55::setQ(const rw::math::Q& q) {
	if(q.size() < 1) {
...
bool SchunkEVG55::setGraspPowerPct(const double pct) {
	if(_connected) {
...
bool SchunkEVG55::status(unsigned int &status) {
	try {
...
void SchunkEVG55::logTextReadySig(const std::string& text, const bool warning) {
	if(warning)
